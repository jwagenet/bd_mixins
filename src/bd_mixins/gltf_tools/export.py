...
def export_wrapper(objects, filename):
    ...
    gltf.materials = []
    for i, object in enumerate(objects.children):
        mat_i = appearances.index(object.appearance)
        gltf_mats = [mat.name for mat in gltf.materials]
        if object.appearance.name not in gltf_mats:
            gltf.materials.append(ConvertMaterial(gltf).add_material(object.appearance))

        gltf.meshes[i].name = object.label
        for primitive in gltf.meshes[i].primitives:
    ...
    # Rebuild node tree to remove intermediate nodes which are empty containers and not the top level node
    # (one parent / one child / no mesh) and corrects node labels.
    # Only supports a flat Compound
    gltf.nodes[0].name = objects.label if objects.label is not None else "Compound"
    new_nodes = [gltf.nodes[0]]
    for node in gltf.nodes[1:]:
        if node.mesh is not None:
            new_nodes.append(node)
            node.name = gltf.meshes[node.mesh].name

    new_nodes[0].children = list(range(1, len(new_nodes), 1))
